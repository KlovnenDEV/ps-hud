...
exports['qbr-core']:AddCommand("dev", "Enable/Disable developer Mode", {}, false, function(source, args)
...
RegisterNetEvent('hud:server:GainStress', function(amount)
    ...
    TriggerClientEvent('QBCore:Notify', src, 9, Lang:t("notify.stress_gain"), 2000, 0, 'hud_textures', 'check')
end)
...
RegisterNetEvent('hud:server:RelieveStress', function(amount)
    ...
    Player.Functions.SetMetaData('stress', newStress)
    TriggerClientEvent('hud:client:UpdateStress', src, newStress)
    TriggerClientEvent('QBCore:Notify', src, 9, Lang:t("notify.stress_removed"), 2000, 0, 'hud_textures', 'check')
end)

RegisterNetEvent('hud:server:saveUIData', function(data)
    local src = source
	-- Check Permissions
    if not exports['qbr-core']:HasPermission(src, 'admin') and not IsPlayerAceAllowed(src, 'command') then
		return
	end
...
exports['qbr-core']:CreateCallback('hud:server:getMenu', function(source, cb)
    cb(Config.Menu)
end) 

exports['qbr-core']:CreateCallback('hud:server:getRank', function(source, cb)
    local src = source
